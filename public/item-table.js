 ...
 * 检查，而列的计算是表格里唯一真有判断的地方——算错一列，整张表要么少一列数据，
 * 要么多一列永远空着的表头，两种都是安静地错。
 *
 * 列不是从数据里的维度全摊开来的：维度是开放集合，插件随时能再贴几个，全摊开就是
 * 一张永远在横滚的表。哪些值得成列是使用者的判断，而这个判断在筛选区已经做过一次
 * 了（「添加字段」），所以表格直接复用它，不发明第二套开关。
 */
