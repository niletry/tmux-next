...
- **API**（`plugins/supervisor/server.ts`）：
  ...
  - `POST /api/supervisor/create-session` — 查重 → 创建会话（复用现有 `session-create` 流程）→ 用
    模板渲染首条 prompt 并 `sendText` 灌入 → 写 `registry.json`。这条路由收的是新建会话页对所有
    "会话类型"插件统一的信封 `{ kind, dir, name, fields }`（见「六、创建流程改动点」），不再是
    这个插件自己发明的请求形状。
- **页面**（`plugins/supervisor/public/`）：按工作区分组的时间线，一行一次巡检——时间、检查了哪
  些会话、每个的状态、有没有介入、介入了什么。这页现在整体只读：创建监察者的入口已经搬到新建会话
  页（见下），这里不再有自己的创建表单。跟其它插件一样自带 `style.css`，颜色只用主题变量。
...
- 前端入口：新建会话页新增一种"会话类型"（`plugins/types.ts` 的 `Plugin.sessionKinds`），选中后
  复用那个页面已有的目录浏览器，只是把"普通会话"才有意义的控件（agent 选择、跳过权限、恢复历史、
  模板选择器）换成插件声明的字段（这里只有一个：是否自动确认权限提示）。内核不为此新增路由或认识
  "监察者"这个名字——它只按插件清单画一个选项，提交时原样 POST 给插件自己的 `/api/<id>/*`。
