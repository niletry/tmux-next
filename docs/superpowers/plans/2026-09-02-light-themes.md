...
Run: `~/.bun/bin/bun test src/themes.test.ts`
Expected: PASS。Latte 的 `--ok`/`--warn`/`--danger` 变成 `#378a25` / `#a96b16` / `#d20f39`，`--border-2` 变成 `#b2b4c0`。

四套深色主题的**边框**逐字节不变（它们第一步就过线，`pushTo` 原样返回起点）。**语义色有一个例外**：`nord` 的 `--danger` 会从 `#cb868e` 变成 `#ce878e`——它的 `ansi[9]`（`#bf616a`）对 `--surface-4` 只有 2.108:1，是 12 个语义色槽位里唯一第一步不过线、因而真的要朝端点插值的一个，端点换了落点就换。对比度 3.0142 → 3.0720，已裁定接受。**不要为了保住这个字节把深色的端点改回 `ansi[15]`**——那会让「远离表面的那一端」有两个互相矛盾的定义，而 `--accent-hover` 明确需要纯白。
