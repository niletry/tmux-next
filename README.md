...
| **Jira** | Optional plugin, and a data source rather than a tab of its own: it syncs the issues assigned to you onto the home page's work-item list, and paints each card with that issue's status, epic, PRs and CI checks. Configured on the settings page |
...
Two of those placeholders both look like "the ticket": `{item.ref}` is the tracker's own key (a Jira issue key like `PROJ-123`) — that's the one to put in a session name or a commit message. `{item.id}` is tmux-next's own internal work-item id, unrelated to the tracker, and rarely what you want in a template.
...
On a wide screen the same list also comes as a **table** — one row per item, with the state, the bound sessions and the actions in fixed columns, plus one column for each field you have added to the filter area. Grouping, filtering and the work filter mean the same thing in both layouts, so switching costs you nothing; group names become heading rows spanning the table. The switch is remembered per device and isn't offered below 900px — scanning a table sideways on a phone defeats the point of one.
