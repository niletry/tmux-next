...
function usedKeys(): Map<string, string[]> {
  ...
  for (const path of files) {
    ...
    //
    // doneKey 是 actions: [...] 那颗按钮的回执文案，跟 labelKey/hintKey 同一条
    // 道理——tr(action.doneKey) 也是动态查找，扫描器只认字面量。
    for (const m of source.matchAll(/\b(?:labelKey|hintKey|doneKey):\s*"([A-Za-z0-9_.]+)"/g)) {
      found.set(m[1], [...(found.get(m[1]) ?? []), file]);
    }
  }
  return found;
