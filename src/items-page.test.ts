 ...
 * 一行一张单，状态一眼扫过去，操作不用去卡片的角上找。所以这是同一份数据的
 * 第二种排法，不是第二个页面：分组、筛选、开工筛选三个控件在两种视图下含义
 * 完全一致，切过去不丢上下文。
 */

const VIEW_KEY = "tmux-next.items.view";
// 内部存储值仍然是 "table"——不给人看的标识，改名字要的是迁移旧值，这里不改。
...
test("存了列表视图时画一行一张单", async () => {
  const root = await mount(payload({ items: twoItems, facets: twoDims }), asTable());
  expect(root.querySelector(".item-card")).toBeNull();
  expect(root.querySelectorAll(".item-list .item-list-row").length).toBe(2);
  expect(root.querySelector(".item-list-row .item-title")?.textContent).toBe("修登录页");
});

// 没有横向滚动容器——状态一侧换行代替横滚，见 style.css 里那段注释。
test("列表视图没有横向滚动容器", async () => {
  const root = await mount(payload({ items: twoItems, facets: twoDims }), asTable());
  expect(root.querySelector(".table-wrap")).toBeNull();
});

// chip 不从数据里的维度全摊开——那是开放集合。哪些值得画的判断在筛选区已经做过
// 一次了，列表直接复用它。
test("已加的筛选字段在列表里各成一颗 chip", async () => {
  const store = { ...asTable(), [FIELDS_KEY]: JSON.stringify(["jira.status"]) };
  const root = await mount(payload({ items: twoItems, facets: twoDims }), store);
  const first = root.querySelector(".item-list-row")!;
  const chips = [...first.querySelectorAll(".row-status .facet")].map((n) => n.textContent);
...
test("一个字段都没加时只有 agent 这一颗 chip", async () => {
  const root = await mount(payload({ items: twoItems, facets: twoDims }), asTable());
  const first = root.querySelector(".item-list-row")!;
  const chips = [...first.querySelectorAll(".row-status .facet")];
  expect(chips.length).toBe(1);
  expect(chips[0]?.textContent).toContain(tr("items.agent.waiting"));
});

// 动作在右、状态在左：操作一侧不该混进任何 facet chip。
test("状态和操作分在两侧", async () => {
  const root = await mount(payload({ items: [item()] }), asTable());
  const row = root.querySelector(".item-list-row")!;
  expect(row.querySelector(".row-status .item-title")).not.toBeNull();
  expect(row.querySelector(".item-actions .facet")).toBeNull();
});

// 分组在列表下也生效：组名是贯通整块的标题，跟卡片分组视图长得一样。
test("分组名在列表里跟卡片视图同一个 .group-name", async () => {
  const root = await mount(payload({ items: twoItems, facets: twoDims }), asTable());
  const heads = [...root.querySelectorAll(".item-list section > .group-name")];
  expect(heads.map((n) => n.textContent)).toEqual([
    tr("items.agent.waiting"), tr("items.agent.working"),
  ]);
});

// 列表不是卡片的只读版：卡片上够得到的东西，列表里也要够得到，否则切过去就是
// 一次功能降级。
test("列表里会话链接还在", async () => {
  const root = await mount(payload({
    items: [item()],
    sessions: [session({ name: "跑测试" })],
    bindings: [{ session: "跑测试", itemId: "it-1", live: true }],
  }), asTable());
  expect(root.querySelector(".item-list-row .item-session")?.textContent).toContain("跑测试");
});

// 动作跟卡片一样分两档：常用的摊在行里，其余收进 ⋯。列表不该自己发明第三种
// 分法——同一个动作在两种视图里藏在不同深度，是最容易让人找不着的那种不一致。
test("列表里的动作跟卡片同一套两档分法", async () => {
  const root = await mount(payload({ items: [item()] }), asTable());
  const row = root.querySelector(".item-list-row")!;
...
test("切到列表视图会存下来", async () => {
  ...
  expect(root.querySelector(".item-list")).not.toBeNull();
});
...
// 筛空这些状态跟视图无关，列表视图下也得照说，不能画一份空列表却什么提示都没有。
test("列表视图下筛空了照样给提示", async () => {
  const store = {
    ...asTable(),
