...
| **会话模板** | 从一张单开会话时，可以用模板预填会话名和首条输入——`{item.title}`、`{jira.summary}` 之类占位符。在设置页里建、改 |
...
| **Jira** | 可选插件，是首页的一个数据源，不占顶栏标签：把分给你的工单同步成首页的单，并给每张卡片贴上这个工单的状态、史诗、PR 与 CI 检查。在设置页里配 |
...
从单的卡片上开会话会先进新建会话页，页里可以弹出一个模板选择器：选一个，会话名和首条输入就按模板填好了，两处都还能改，占位符像 `{item.title}`、`{item.ref}`，或者插件自己的字段（`{jira.summary}`、`{jira.description}`）。一行里的占位符如果全渲染成空，这一整行就删掉；没有占位符的行永远保留。模板在设置页里建、改，页面上会列出当前可用的占位符供点选——一条没建就不存在，没有模板时选择器也就不出现。

两个占位符都像是"单号"，容易选错：`{item.ref}` 才是外部系统自己的编号（比如 Jira 的 `PROJ-123`），会话名、首条输入里想带工单号用这个。`{item.id}` 是 tmux-next 自己的内部工作项 id，跟 Jira 无关，模板里基本用不上。
...
宽屏上同一份列表还能切成**表格**：一张单一行，状态、绑定的会话、操作各占固定列，此外你在筛选区加过的每个字段各成一列。分组、筛选、开工筛选在两种排法下含义完全一致，切过去不丢上下文；分组名在表格里是一行贯通全表的标题行。这个选择逐设备记住，900px 以下不提供——手机上横着扫一张表，正好把选表格的理由抵消掉。
