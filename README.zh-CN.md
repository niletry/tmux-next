...
每张单的卡片上有一行 chip：`waiting`/`working`/`none`、绑定的会话数、工作目录、标签，以及插件贴上来的任何东西（Jira 的状态、史诗、PR、检查）。每个 chip 都是一对 `{ 维度, 取值 }`，首页可以按选中的维度分组或筛选；可选项直接从当前卡片上现读，不是写死的表，所以一个新插件维度冒出来不需要改这个页面。分组和筛选的选择逐设备记住，不跨设备同步。没有绑定任何会话的单也照样有卡片，落进「未归单」那一组——没有 facet 不代表从列表里消失。
...
每套配色都由测试按 WCAG AA 校验。要说明的是，四套预设都**没有**沿用各自官方的 `brightBlack`：它们相对自身背景只有 1.7:1 到 2.5:1，而那正是 Claude Code 用来画次要信息的颜色，手机拿到户外根本看不清。各自换成了同一官方色板里更亮的一档。
