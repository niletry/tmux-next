...
从单的卡片上开会话时，可以先弹出一个模板选择器：选一个，还没进新建会话页，会话名和首条输入就已经按模板填好了，占位符像 `{item.title}`、`{item.ref}`，或者插件自己的字段（`{jira.summary}`、`{jira.description}`）。一行里的占位符如果全渲染成空，这一整行就删掉；没有占位符的行永远保留。模板在设置页里建、改，页面上会列出当前可用的占位符供点选——一条没建就不存在，没有模板时选择器也就不出现。
