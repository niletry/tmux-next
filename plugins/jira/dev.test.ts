...
import { fetchDev, parsePrUrl } from "./dev";
...
test("两跳都通时，PR 带着它的检查回来", async () => {
  const res = await fetchDev(CONFIG, "10001", routed({ dev: [200, DEV_BODY], status: [200, STATUS_BODY] }));
...
test("dev-status 按 issue id 查，不是按 key", async () => {
  const seen: string[] = [];
  await fetchDev(CONFIG, "10001", routed({ dev: [200, DEV_BODY], status: [200, STATUS_BODY], seen }));
...
test("Bitbucket 那一跳挂了，PR 仍然回来，只是检查标成没问到", async () => {
  ...
  const res = await fetchDev(CONFIG, "10001", routed({ dev: [200, DEV_BODY], status: [500, "boom"] }));
  expect(res.ok).toBe(true);
  if (!res.ok) return;
...
test("没配 Bitbucket 时不去问，检查同样标成没问到", async () => {
  ...
  const res = await fetchDev(noBb as JiraConfig, "10001", routed({ dev: [200, DEV_BODY], seen }));
  expect(res.ok).toBe(true);
  if (!res.ok) return;
  expect(res.prs[0]!.checksKnown).toBe(false);
...
test("「没有检查」与「没问到检查」是两种结果", async () => {
  ...
  const res = await fetchDev(CONFIG, "10001", routed({ dev: [200, DEV_BODY], status: [200, { values: [] }] }));
  expect(res.ok).toBe(true);
  if (!res.ok) return;
  expect(res.prs[0]!.checks).toEqual([]);
...
test("没有 PR 是正常答案，不是错误", async () => {
  const res = await fetchDev(CONFIG, "10001", routed({ dev: [200, { detail: [{ pullRequests: [] }] }] }));
  expect(res).toEqual({ ok: true, prs: [] });
});

test("缺编号或地址的 PR 被跳过，而不是渲染成一行噪音", async () => {
  const body = { detail: [{ pullRequests: [{ name: "没有 id" }, DEV_BODY.detail[0]!.pullRequests[0]] }] };
  const res = await fetchDev(CONFIG, "10001", routed({ dev: [200, body], status: [200, STATUS_BODY] }));
  expect(res.ok && res.prs.map((p) => p.id)).toEqual(["371"]);
});

test("Jira 那一跳的 401 报凭据无效", async () => {
  expect(await fetchDev(CONFIG, "10001", routed({ dev: [401, { errorMessages: ["x"] }] }))).toEqual({
    ok: false,
    reason: "auth",
  });
});
